...
def generate_reports_data(db: Session, psychologist_id: int) -> ReportsData:
    ...
    completed_sessions = len([apt for apt in appointments if apt.statsus == AppointmentStatus.CONCLUIDO])
    canceled_sessions = len([apt for apt in appointments if apt.statsus == AppointmentStatus.CANCELADO])
    scheduled_sessions = len([apt for apt in appointments if apt.statsus == AppointmentStatus.AGENDADO])
    ...
    if patients_with_sessions > 0:
        patients_data.append(StatusData(name="Sem sessões", value=patients_without_sessions, clor="#EF4444"))
